    ...
    <main class="control-main">
      ...
      <section v-if="activeSection === 'overview'" class="page-section">
        <div class="hero-panel">
          ...
          <div class="hero-orbit"><div class="orbit-core">VS</div><i v-for="n in 12" :key="n" :style="{ transform: `rotate(${n * 30}deg)` }"></i></div>
        </div>
      ...
      <section v-else-if="activeSection === 'players'" class="page-section">
        <div class="section-heading"><div><span>PLAYER DATABASE</span><h2>玩家名单</h2><p>头像会被保存在本机，并在所有比赛快照中使用。</p></div><form class="inline-create" @submit.prevent="createPlayer"><input v-model="newPlayerName" maxlength="32" placeholder="输入玩家名称" /><button class="primary" :disabled="busy === 'player'">＋ 添加玩家</button></form></div>
        <div v-if="players.length" class="player-grid">
          <article v-for="(player, index) in players" :key="player.id" class="player-card">
            <div class="avatar-wrap">
              ...
              <label title="上传头像">⌁<input type="file" accept="image/png,image/jpeg,image/webp" @change="uploadAvatar(player, $event)" /></label>
            </div>
